...
namespace {
	...
	public:
		StatementExprAST(std::vector<std::unique_ptr<ExprAST>> statement)
...
static std::unique_ptr<ExprAST>ParseBlock() {
	...
	getNextToken();
	while (CurTok == tok_VAR) {
		statement.push_back(ParseVarExpr());
	};
	while (CurTok == tok_var || CurTok == tok_RETURN || CurTok == tok_PRINT || CurTok == '{') {
		statement.push_back(ParseStatement());
	};
	if (CurTok != '}')
		return LogError("Expected '}' in Expression");
	return llvm::make_unique<StatementExprAST>(std::move(statement));
}
static std::unique_ptr<ExprAST> ParseStatement() {
	...
	case tok_var:
		...
		break;
	case tok_RETURN:
		statement.push_back(ParseReturn());
		break;
	case tok_PRINT:
		statement.push_back(ParsePrint());
		break;
	case '{':
		statement.push_back(ParseBlock());
		break;
	}
