...
namespace {
	...
	public:
		PrintExprAST(std::string Text, std::vector<std::unique_ptr<ExprAST>> expr) :
...
static std::unique_ptr<ExprAST> ParseExpression1();//step null
...
static std::unique_ptr<ExprAST> ParseNumberExpr1() {
...
static std::unique_ptr<ExprAST> ParseWhileExpr() {
	...
	auto Start = ParseExpression1();
	if (!Start)
		return nullptr;

	std::unique_ptr<ExprAST> Step;
	Step = ParseExpression1();
	if (!Step) {
		return nullptr;
	}
...
static std::unique_ptr<ExprAST> ParsePrintExpr()
{
	...
	return llvm::make_unique<PrintExprAST>(printtext, std::move(expr));
}
...
static std::unique_ptr<ExprAST> ParseExpression1() {
	auto LHS = ParseNumberExpr1();
	if (!LHS)
		return nullptr;
	return ParseBinOpRHS(0, std::move(LHS));
}
...
Value *PrintExprAST::codegen() {
	...
	Builder.CreateCall(TheFunction, paramArrayRef);
